...
func.func @reshape_canonicalize_const() -> tensor<1x5xi32> {
  // CHECK: %[[VAR0:.+]] = "tosa.const"() <{value = dense<[0, 1, 2, 3, 4]> : tensor<5xi32>}>
  // CHECK: %[[VAR1:.+]] = tosa.const_shape {value = dense<[1, 5]> : tensor<2xindex>} : () -> !tosa.shape<2>
  // CHECK: %[[VAR2:.+]] = tosa.reshape %[[VAR0]], %[[VAR1]]
  // CHECK: return %[[VAR2]]
