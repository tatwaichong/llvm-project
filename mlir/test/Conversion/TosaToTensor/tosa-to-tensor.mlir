...
func.func @test_reshape_samerank_unsigned(%arg0: tensor<3x2xui8>) -> tensor<2x3xui8> {
  // CHECK: %[[VAL_1:.*]] = tosa.const_shape  {value = dense<[2, 3]> : tensor<2xindex>} : () -> !tosa.shape<2>
  // CHECK: %[[VAL_2:.*]] = tensor.collapse_shape %[[VAL_0]] {{\[\[}}0, 1]] : tensor<3x2xui8> into tensor<6xui8>
  // CHECK: %[[VAL_3:.*]] = tensor.expand_shape %[[VAL_2]] {{\[\[}}0, 1]] output_shape [2, 3] : tensor<6xui8> into tensor<2x3xui8>
  %s = tosa.const_shape { value = dense<[2, 3]> : tensor<2xindex> } : () -> !tosa.shape<2>
